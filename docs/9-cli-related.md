...
In order to automatically discovering all commands within each application,

we need some modules, like `inspect` and `importlib`.

For example, a command is located in `src/apps/rbac/commands/filename.py`.

Do not place commands in packages like `commands/app1`, `commands/app2`, etc.
