...
async def email_verification(
    req: EmailVerificationRequestSchema, cache: Redis = Depends(get_cache), _: Any = Depends(RateLimitDep(1, 60))
) -> JSONResponse:
    resp_dict = await EmailVerificationService.verify(req=req.model_dump(), cache=cache)
